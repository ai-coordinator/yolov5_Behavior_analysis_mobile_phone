                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if label[0] == 'c':
                            ...
                            # cv2.putText(im0, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tl, lineType=cv2.LINE_AA)

                            # AI coordinator
                            for face in faces:
                                # rectangle visualize
                                a_x1, a_y1, a_x2, a_y2 = int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])
                                b_x1, b_y1, b_x2, b_y2 = face.left(), face.top(), face.right(), face.bottom()
                                ...
                                if Overlapping > 0:
                                    cv2.putText(im0, 'Calling', (c1[0], c1[1] - 2), 0, tl, [100, 100, 255], thickness=tl, lineType=cv2.LINE_AA)
                                else:
                                    color = (255,0,0)
                                    # cv2.rectangle(im0, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
                                    cv2.putText(im0, 'Not calling', (c1[0], c1[1] - 2), 0, tl, [225, 255, 255], thickness=tl, lineType=cv2.LINE_AA)


                                # cv2.rectangle(im0, pt1=(face.left(), face.top()), pt2=(face.right(), face.bottom()),
